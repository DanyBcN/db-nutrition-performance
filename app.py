...
if menu == "Nuova Valutazione":
    ...
    cog_input = st.text_input("Cognome")

    suggestions = [c for c in cognomi if cog_input.lower() in c.lower()]
    if suggestions:
        st.info(f"Cognome già presente: {suggestions}")
    ...
    if st.button("Genera Analisi"):
        ...
        if st.button("Salva in Archivio"):
            with get_connection() as conn:
                cur = conn.cursor()
                cur.execute("INSERT INTO atleti (nome,cognome,altezza,profilo) VALUES (?,?,?,?)",
                            (nome,cog_input,altezza,profilo))
                atleta_id = cur.lastrowid
                cur.execute("""INSERT INTO visite 
                            (atleta_id,data,peso,fm,ftp,lthr,peso_t,fm_t,ftp_t,dist,grad,bike,t_att,t_tar)
                            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                            (atleta_id,str(data_analisi),peso,fm,ftp,lthr,peso_t,fm_t,ftp_t,dist,grad,bike,t_att,t_tar))
                conn.commit()
            st.success("Salvato")
...
if menu == "Archivio":
    ...
    if not atleti.empty:
        ...
        if not visite.empty:
            st.subheader("Confronto Visite")
            sel_vis = st.multiselect("Seleziona visite",visite['id'])
            if len(sel_vis)>=2:
                df = visite[visite['id'].isin(sel_vis)]
                st.dataframe(df[['data','peso','fm','ftp']])
