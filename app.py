...
# ---------------------------------------------------------
# LOGICA CLINICA E CARATTERI UNICODE
# ---------------------------------------------------------
class NutritionScience:
    ...
    def get_clinical_judgment(profilo, peso, altezza, fm, fm_t):
        ...
        # Testo con caratteri Unicode professionali
        msg = f"--- ANALISI BIOMETRICA ---\n"
        msg += f"BMI: {bmi:.1f} kg/m² | FM: {fm}% | FFMI: {ffmi:.1f}\n"
        msg += f"Massa Magra (FFM): {ffm:.1f} kg\n\n"
        msg += f"Valutazione per profilo {profilo}:\n"
        
        if fm > 15:
            msg += "Si consiglia focus su ricomposizione corporea per ottimizzare il rapporto W/kg."
        else:
            msg += "Composizione corporea ottimale per la categoria. Focus su VO₂max e soglia."
        return msg
...
if menu == "Nuova Analisi":
    ...
    with st.expander("👤 Anagrafica", expanded=True):
        ...
        cog_sel = c1.selectbox("Cognome registrato", [""] + sorted(db_atleti['cognome'].unique().tolist()))
        ...
        data_v = c3.date_input("Data", date.today())
    ...
    with col2:
        st.subheader("🎯 Target")
        peso_t, fm_t = st.number_input("Peso Target", 40.0, 150.0, 68.0), st.number_input("FM Target %", 3.0, 40.0, 10.0)
        ftp_t = st.number_input("FTP Target", 50, 600, 270)
    with col3:
        st.subheader("🏔️ Scenario")
        dist_km, grad = st.number_input("Km Salita", 0.1, 50.0, 10.0), st.number_input("Pendenza %", 0.0, 20.0, 7.0)
        bike_w = st.number_input("Peso Bici", 5.0, 15.0, 8.0)

    if st.button("🚀 CALCOLA E GENERA REPORT"):
        ...
        st.session_state['res'] = {
            ...
            't_a': t_a, 't_t': t_t, 'd': dist_km, 'g': grad, 'b': bike_w, 'giudizio': giudizio
        }
    ...
    if 'res' in st.session_state:
        ...
        # Zone
        zp = [("Z1", 0, int(r['ftp_t']*0.55)), ("Z2", int(r['ftp_t']*0.56), int(r['ftp_t']*0.75)), ("Z3", int(r['ftp_t']*0.76), int(r['ftp_t']*0.90)), ("Z4", int(r['ftp_t']*0.91), int(r['ftp_t']*1.05)), ("Z5", int(r['ftp_t']*1.06), 600)]
        st.write("### ⚡ Zone Potenza Target")
        st.table(pd.DataFrame(zp, columns=["Zona", "Min (W)", "Max (W)"]))
        ...
        with c_pd:
            ...
            pdf.set_font("Arial", '', 10); pdf.cell(190, 7, f"Data: {r['data']} | Altezza: {r['alt']}cm | Profilo: {r['prof']}", 0, 1, 'C'); pdf.ln(10)
            
            pdf.set_fill_color(230, 230, 230); pdf.set_font("Arial", 'B', 12); pdf.cell(190, 10, "VALUTAZIONE CLINICA", 1, 1, 'L', True)
            pdf.set_font("Arial", '', 11); pdf.multi_cell(190, 8, pdf_safe(r['giudizio']), 1); pdf.ln(5)
            
            pdf.set_font("Arial", 'B', 12); pdf.cell(190, 10, "PERFORMANCE IN SALITA", 1, 1, 'L', True)
            ...
            st.download_button("📄 SCARICA REPORT PDF", data=pdf.output(dest='S').encode('latin-1'), file_name=f"Report_{r['c']}.pdf")
    ...
    if not at.empty:
        ...
        with st.expander("📝 Modifica Anagrafica"):
            ...
            if st.button("Salva Modifiche"):
                with get_connection() as conn:
                    conn.execute("UPDATE atleti SET nome=?, cognome=?, altezza=? WHERE id=?", (un, uc, ua, a_id))
                    conn.commit()
                st.rerun()
        ...
        if not vi.empty:
            ...
            scelte = st.multiselect("Seleziona visite da confrontare", vi['label'].tolist(), default=vi['label'].tolist()[:min(2, len(vi))])
            
            if len(scelte) >= 2:
                df_c = vi[vi['label'].isin(scelte)].sort_values(by='data')
                v1, v2 = df_c.iloc[0], df_c.iloc[-1]
                st.subheader(f"Confronto: {v1['data']} vs {v2['data']}")
