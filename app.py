...
# =========================================================
    ...
    atleti = atleti.copy()
    atleti["label"] = atleti.apply(lambda x: f"{x['id']} - {x['cognome']} {x['nome']}", axis=1)
    selected = st.selectbox("Seleziona atleta", atleti["label"].tolist())
    ...
    edit_cognome = a1.text_input("Cognome atleta", value=str(atleta_row["cognome"]), key=f"edit_cognome_{atleta_id}")
    edit_nome = a2.text_input("Nome atleta", value=str(atleta_row["nome"]), key=f"edit_nome_{atleta_id}")
    edit_altezza = a3.number_input("Altezza atleta (cm)", 120, 230, int(atleta_row["altezza"]) if pd.notna(atleta_row["altezza"]) else 175, 1, key=f"edit_altezza_{atleta_id}")
    edit_profilo = a4.selectbox("Profilo atleta", PROFILI, index=PROFILI.index(atleta_row["profilo"]) if atleta_row["profilo"] in PROFILI else 0, key=f"edit_profilo_{atleta_id}")
    ...
    if visite.empty:
        ...
        if len(visite) >= 2:
            ...
            if visita_1_id == visita_2_id:
                ...
                comp_dist = s1.number_input("Km salita comparativa", 0.1, 100.0, float(v2["dist_km"]) if pd.notna(v2["dist_km"]) else 10.0, 0.1, key="comp_dist")
                comp_grad = s2.number_input("Pendenza media comparativa (%)", 0.0, 25.0, float(v2["grad"]) if pd.notna(v2["grad"]) else 7.0, 0.1, key="comp_grad")
                comp_bike = s3.number_input("Peso bici comparativo (kg)", 5.0, 20.0, float(v2["bike_w"]) if pd.notna(v2["bike_w"]) else 7.5, 0.1, key="comp_bike")
                ...
                p1.metric("Peso", f"{peso_2:.1f} kg", f"{delta_peso:+.1f} kg")
                p2.metric("FM %", f"{fm_2:.1f} %", f"{delta_fm:+.1f} %")
                p3.metric("FTP", f"{ftp_2:.0f} W", f"{delta_ftp:+.0f} W / {perc_ftp:+.1f}%")
                p4.metric("W/kg", f"{wkg_2:.2f}", f"{delta_wkg:+.2f} / {perc_wkg:+.1f}%")

                st.markdown("### 🏔️ Comparazione sulla stessa salita")
                a, b, c = st.columns(3)
                a.info(f"**Analisi iniziale**  \nData: {v1['data']}  \nPeso: {peso_1:.1f} kg  \nFTP: {ftp_1:.0f} W  \nW/kg: {wkg_1:.2f}  \nTempo stimato: **{tempo_1:.2f} min**")
                b.success(f"**Analisi successiva**  \nData: {v2['data']}  \nPeso: {peso_2:.1f} kg  \nFTP: {ftp_2:.0f} W  \nW/kg: {wkg_2:.2f}  \nTempo stimato: **{tempo_2:.2f} min**")

                if delta_tempo < 0:
                    c.success(f"**Esito:** MIGLIORAMENTO  \nTempo ridotto di {abs(delta_tempo):.2f} min ({abs(perc_tempo):.1f}%).")
                elif delta_tempo > 0:
                    c.error(f"**Esito:** PEGGIORAMENTO  \nTempo aumentato di {delta_tempo:.2f} min ({perc_tempo:.1f}%).")
                else:
                    c.info("**Esito:** INVARIATO  \nTempo stimato invariato.")
