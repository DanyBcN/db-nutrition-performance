...
if st.button("Genera PDF"):
    ...
    pdf.cell(95,6,f"Nome: {nome} {cognome}", border=1)
    pdf.cell(95,6,f"Sesso: {sesso}", border=1, ln=True)

    pdf.cell(95,6,f"Nato a: {comune} ({provincia})", border=1)
    pdf.cell(95,6,f"Data nascita: {data_nascita.strftime('%d/%m/%Y')}", border=1, ln=True)

    pdf.cell(95,6,f"Email: {email}", border=1)
    pdf.cell(95,6,f"Telefono: {telefono}", border=1, ln=True)

    pdf.cell(190,6,f"Indirizzo: {indirizzo}", border=1, ln=True)
    ...
    pdf.cell(63,6,f"Peso: {peso:.2f} kg", border=1)
    pdf.cell(63,6,f"Altezza: {altezza:.0f} cm", border=1)
    pdf.cell(64,6,f"BMI: {bmi:.2f}", border=1, ln=True)

    pdf.cell(95,6,f"Classificazione: {classificazione}", border=1)
    pdf.cell(95,6,f"Massa grassa: {fm_kg:.2f} kg", border=1, ln=True)

    pdf.cell(190,6,f"Massa magra: {massa_magra:.2f} kg", border=1, ln=True)

    pdf.ln(8)

    pdf.set_font("Arial","B",12)
    pdf.cell(0,8,"PERFORMANCE", ln=True)
    pdf.set_font("Arial", size=10)

    pdf.cell(95,6,f"Metodo FTP: {metodo}", border=1)
    pdf.cell(95,6,f"FTP: {ftp:.2f} W", border=1, ln=True)

    pdf.cell(95,6,f"W/kg: {wkg:.2f}", border=1)
    pdf.cell(95,6,f"FTHR: {fthr:.0f} bpm", border=1, ln=True)
    ...
    if zone_potenza_df is not None:
        ...
        pdf.cell(40,6,"Zona", border=1)
        pdf.cell(75,6,"Da (W)", border=1)
        pdf.cell(75,6,"A (W)", border=1, ln=True)

        for _, row in zone_potenza_df.iterrows():
            pdf.cell(40,6,str(row["Zona"]), border=1)
            pdf.cell(75,6,str(row["Da (W)"]), border=1)
            pdf.cell(75,6,str(row["A (W)"]), border=1, ln=True)
    ...
    if zone_cardio_df is not None:
        ...
        pdf.cell(40,6,"Zona", border=1)
        pdf.cell(75,6,"Da (bpm)", border=1)
        pdf.cell(75,6,"A (bpm)", border=1, ln=True)

        for _, row in zone_cardio_df.iterrows():
            pdf.cell(40,6,str(row["Zona"]), border=1)
            pdf.cell(75,6,str(row["Da (bpm)"]), border=1)
            pdf.cell(75,6,str(row["A (bpm)"]), border=1, ln=True)
    ...
    pdf.cell(95,6,f"Nuovo peso: {nuovo_peso:.2f} kg", border=1)
    pdf.cell(95,6,f"Nuova FTP: {nuova_ftp:.2f} W", border=1, ln=True)

    pdf.cell(190,6,f"Nuovo W/kg: {nuovo_wkg:.2f}", border=1, ln=True)

    pdf.output("report_professionale.pdf")

    with open("report_professionale.pdf","rb") as f:
        st.download_button("Scarica PDF Professionale",f,"report_professionale.pdf")import streamlit as st
from datetime import date
from PIL import Image
from fpdf import FPDF
import pandas as pd
