...
def init_db():
    with get_connection() as conn:
        ...
        # Tabella Visite (con tutte le colonne necessarie)
...
class BioPerformance:
    ...
    def estimate_time(watt, peso, km, pend, bike_w):
        # Fisica applicata: stima tempo scalata (resistenza gravitazionale prevalente)
...
if menu == "➕ Nuova Valutazione":
    ...
    st.subheader("📊 Parametri Rilevati e Target")
        ...
        with c_btn1:
            if st.button("💾 SALVA IN ARCHIVIO"):
                with get_connection() as conn:
                    ...
                    conn.commit()
                st.balloons()
                st.info("Dati salvati in database.")

        with c_btn2:
            pdf = FPDF()
            pdf.add_page()
            pdf.set_font("Arial", 'B', 16); pdf.cell(190, 10, pdf_safe(f"REPORT: {r['nome']} {r['cognome']}"), 0, 1, 'C')
            pdf.set_font("Arial", '', 10); pdf.cell(190, 7, f"Data: {r['data']} | Profilo: {r['prof']}", 0, 1, 'C'); pdf.ln(10)
            ...
            pdf.cell(190, 8, pdf_safe(f"Scenario: {r['dist']}km al {r['grad']}% (Bici: {r['bike']}kg)"), 1, 1)
            pdf.cell(95, 8, f"Tempo Attuale: {r['t_a']:.2f} min", 1, 0); pdf.cell(95, 8, f"Tempo Target: {r['t_t']:.2f} min", 1, 1)
            
            pdf.ln(5); pdf.set_font("Arial", 'B', 12); pdf.cell(190, 10, "ZONE DI ALLENAMENTO TARGET", 1, 1, 'L', True)
            for z, mi, ma in zp: pdf.cell(190, 7, f"{z}: {mi} - {ma} Watt", 1, 1)
            
            st.download_button("📄 SCARICA REPORT COMPLETO", data=pdf.output(dest='S').encode('latin-1'), file_name=f"Report_{r['cognome']}.pdf")
...
# --- SEZIONE: ARCHIVIO PROFESSIONALE ---
    ...
    if not at.empty:
        ...
        with st.expander("⚙️ Modifica o Elimina Atleta"):
            ...
            up = c4.selectbox("Profilo", ["Scalatore", "Passista", "Triatleta", "Granfondista"], 
                              index=["Scalatore", "Passista", "Triatleta", "Granfondista"].index(curr['profilo']) if curr['profilo'] else 0)
            
            cb1, cb2 = st.columns(2)
            if cb1.button("✅ SALVA MODIFICHE ANAGRAFICA"):
                with get_connection() as conn:
                    conn.execute("UPDATE atleti SET nome=?, cognome=?, altezza=?, profilo=? WHERE id=?", (un, uc, ua, up, a_id))
                    conn.commit()
                st.success("Dati aggiornati!"); st.rerun()
            
            if cb2.button("⚠️ ELIMINA ATLETA (AZIONE IRREVERSIBILE)"):
                with get_connection() as conn:
                    conn.execute("DELETE FROM visite WHERE atleta_id=?", (a_id,))
                    conn.execute("DELETE FROM atleti WHERE id=?", (a_id,))
                    conn.commit()
                st.warning("Atleta rimosso."); st.rerun()
